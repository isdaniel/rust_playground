...
fn main() {
    ...
    for status in list_statuses {
        match status {
            Status::Value(value) if value < 3 => println!("Less than 3"),
            Status::Value(value) if value == 4 => println!("Value is 4"),
            Status::Value(value) => println!("Value is {}", value),
            Status::Stop => println!("Stop!"),
        }
    }
}
